...
class ImageProcessor(QWidget):
    ...
    # Функции
    def filt_sep(self):
        if self.design.label.pixmap():  # Проверяем есть ли изображение для обработки
            conn = sqlite3.connect('imagehistory.db')
            cursor = conn.cursor()
            cursor.execute(
                '''CREATE TABLE IF NOT EXISTS greetings (text filters)''')
            name = "сепия"
            cursor.execute("INSERT INTO greetings (text) VALUES (?)", (name,))
        else:
            pass
